...
// A class template implementing an AVL tree
//
...
protected:
    ...
    // a child removed.
    void _retrace_deletion(shared_ptr<node> subtree_root, int balance_factor_change);
...
template<typename K, typename V>
bool AvlTree<K, V>::exists(const K &key) const {
    shared_ptr<node> found_node = _node_search(key);
...
template<typename K, typename V>
void AvlTree<K, V>::_retrace_deletion(shared_ptr<node> subtree_root, int balance_factor_change) {
...
void AvlTree<K, V>::node::correct_balance() {
    if (left_child && right_child) {
        if (abs(left_child->balance_factor) == abs(right_child->balance_factor))
            balance_factor = 0;
